import { iPhoneProductPageImages, macProductPageImages, iMacProductPageImages, airPodsProductPageImages, airPodsMaxProductPageImages, appRoutes, } from "./consts";
export const matchStaticImages = (brand, model) => {
    const loweredCaseBrand = brand.toLowerCase();
    const isIMac = model === "iMac" || model === "iMac-Pro" || model === "Mac-Studio";
    const isAirPodsMax = model === "AirPods-Max";
    if (loweredCaseBrand === appRoutes.iPhone)
        return iPhoneProductPageImages;
